...
function HomeContent({ needsSignIn }: { needsSignIn: boolean }) {
  ...
  // Route an organizer toward creating a wall: signed-out → sign-in (remembering
  // they wanted /create); demo/mock → straight to the create surface.
  function goCreate() {
    if (needsSignIn) nav("/sign-in", { state: { from: "/create" } });
    else nav("/create");
  }
  ...
  return (
    <div style={{ background: BODY_BG, color: INK, width: "100%", maxWidth: "100vw", overflowX: "hidden" }}>
      ...
      >
        ...
        {/* Header: wordmark left, quiet host link right (never the hero's star). */}
        <header style={{ position: "relative", zIndex: 2, width: "100%", maxWidth: 1180, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
          ...
          <button onClick={goCreate} style={{ background: "transparent", border: "none", color: MUTED, fontWeight: 700, fontSize: 13.5, cursor: "pointer", fontFamily: "inherit", padding: 6 }}>
            Hosting an event? →
          </button>
        </header>
